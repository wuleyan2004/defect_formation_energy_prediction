...
class CrystalNoTransformer(nn.Module):
    def __init__(self, atom_fea_len=9, hidden_dim=128, n_local_layers=3):
        super().__init__()
        ...
        # 3. Global Path (Transformer) - REMOVED
    ...
    def forward(self, batch_dict):
        ...
        # dist_matrix = batch_dict['dist_matrix'] # Not used anymore
...
if __name__ == "__main__":
    ...
    # Try to import dataset, otherwise run with dummy data
    try:
        ...
        if os.path.exists(DATA_PATH) and os.path.exists(FEATURE_PATH):
            dataset = CrystalGraphDataset(DATA_PATH, FEATURE_PATH)
            loader = DataLoader(dataset, batch_size=2, shuffle=True, collate_fn=collate_fn)
            
            model = CrystalNoTransformer(atom_fea_len=9, hidden_dim=64, n_local_layers=2)
        ...
        print(f"Running with dummy data because: {e}")
        # Dummy data test
        B, N = 2, 5
        hidden_dim = 9
        dummy_batch = {
            ...
            'num_atoms': torch.tensor([N, N]),
            'edge_index_list': [torch.randint(0, N, (2, 10)) for _ in range(B)],
            'edge_dist_list': [torch.rand(10) for _ in range(B)],
            'triplet_index_list': [torch.randint(0, N, (5, 3)) for _ in range(B)],
            'angles_list': [torch.rand(5) for _ in range(B)],
            'target': torch.randn(B)
        }
        
        model = CrystalNoTransformer(atom_fea_len=hidden_dim, hidden_dim=64, n_local_layers=2)
