...
# If this script is in src/, dirname(abspath) is src/, dirname(dirname) is root/
...
try:
    ...
    try:
        ...
        # Fallback to search if the file is in a different location relative to execution
...
def main():
    ...
    CONFIG = {
        ...
        'output_dir': '/Users/wuleyan/Desktop/大创_我自己的代码保留一份/Defect_Formation_Energy_Prediction/checkpoints/original_notransformer',
        'batch_size': 32,      
        'epochs': 50,
        'lr': 1e-4,            
        'hidden_dim': 64,      
        'n_local': 2,
        'n_global': 1, # Kept for config consistency, though not used in NoTransformer
        'seed': 42,
        'resume_path': '/Users/wuleyan/Desktop/大创_我自己的代码保留一份/Defect_Formation_Energy_Prediction/checkpoints/original_notransformer/latest_model.pth'
    }
    ...
    # Using CrystalNoTransformer
    model = CrystalNoTransformer(
        atom_fea_len=9,
        hidden_dim=CONFIG['hidden_dim'],
        n_local_layers=CONFIG['n_local']
    ).to(device)
    logger.info(f"Model initialized: CrystalNoTransformer, Hidden Dim={CONFIG['hidden_dim']}, Local Layers={CONFIG['n_local']}")
