...
try:
    ...
    try:
        from WLY.data_loader import CrystalGraphDataset, collate_fn
         ...
         # Try local import if data is in src or relative
         from data_loader import CrystalGraphDataset, collate_fn

try:
    # Import the No-Transformer model
    # Since we added src to sys.path, we can import directly
    from model_no_transformer import CrystalNoTransformer
except ImportError:
    try:
        from src.model_no_transformer import CrystalNoTransformer
    except ImportError:
        print("Error: Could not import CrystalNoTransformer. Please check your python path.")
        sys.exit(1)
...
def split_dataset(dataset, train_ratio=0.8, val_ratio=0.1, seed=42):
    ...
    3. Train set: Includes ONLY Original versions of train IDs.
...
def main():
    ...
    checkpoint_dir = '/Users/wuleyan/Desktop/大创_我自己的代码保留一份/Defect_Formation_Energy_Prediction/checkpoints/original_notransformer'
    
    # Determine checkpoint path (prefer best_model.pth, fallback to latest_model.pth)
    ...
    if os.path.exists(best_ckpt):
        ...
        print(f"Warning: 'best_model.pth' not found, using '{os.path.basename(latest_ckpt)}' instead.")
    else:
    ...
    # n_global is not used in CrystalNoTransformer
    
    print(f"Model Config: Hidden Dim={hidden_dim}, Local Layers={n_local} (No Transformer)")
    
    model = CrystalNoTransformer(
        atom_fea_len=9,
    ...
    try:
        if len(all_targets) > 0:
            ...
            plt.title(f'Test Set Parity Plot (No Transformer)\nMAE={mae:.3f} eV, RMSE={rmse:.3f} eV')
            plt.grid(True, which='both', linestyle='--', linewidth=0.5)
            plt.axis('square')
            
            output_plot = os.path.join(os.path.dirname(CONFIG['checkpoint_path']), 'test_parity_plot_notransformer.png')
