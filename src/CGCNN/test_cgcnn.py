...
# --- Model Definitions (Copied from train_cgcnn.py) ---
...
def split_dataset(dataset, train_ratio=0.8, val_ratio=0.1, seed=42):
    ...
    # Build final indices
    # We only need TEST indices here
...
def get_test_predictions():
    ...
    CONFIG = {
        ...
        'checkpoint_path': '/Users/wuleyan/Desktop/大创_我自己的代码保留一份/Defect_Formation_Energy_Prediction/checkpoints/CGCNN/best_model.pth',
        'batch_size': 32,
        'seed': 42
    }
    ...
    n_conv = saved_config.get('n_conv', 4)
    
    print(f"Model Config: Hidden Dim={hidden_dim}, Conv Layers={n_conv}")
...
def main():
    ...
    try:
        if len(all_targets) > 0:
            ...
            plt.scatter(all_targets, all_preds, alpha=0.5, s=10, c='purple', edgecolors='none')
            ...
            plt.title(f'CGCNN Test Parity\nMAE={mae:.3f} eV, RMSE={rmse:.3f} eV')
            plt.grid(True, which='both', linestyle='--', linewidth=0.5)
            plt.axis('square')
            
            output_plot = '/Users/wuleyan/Desktop/大创_我自己的代码保留一份/Defect_Formation_Energy_Prediction/Final_test_cgcnn.png'
